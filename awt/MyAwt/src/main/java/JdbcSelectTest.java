...
public class JdbcSelectTest {   // Save as "JdbcSelectTest.java"
    public static void main(String[] args) {
        try (
            ...
            String strSelect = "select seqno, name, email,id from user";
            ...
            while(rset.next()) {   // Repeatedly process each row
                int seqno = rset.getInt("seqno");  // retrieve a 'String'-cell in the row
                String name = rset.getString("name");  // retrieve a 'double'-cell in the row
                String email  = rset.getString("email");       // retrieve a 'int'-cell in the row
                int id  = rset.getInt("id");       // retrieve a 'int'-cell in the row
                System.out.println(seqno + ", " + name + ", " + email + ", " + id);
                ++rowCount;
            }
